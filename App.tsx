...
`;
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-900 via-stone-900 to-black animate-gradient flex flex-col items-center justify-between p-4 sm:p-6 font-sans">
      <main className="w-full max-w-md mx-auto flex flex-col items-center justify-center flex-grow">
        <div className="w-full rounded-3xl p-px bg-animated-border shadow-2xl shadow-stone-800/20">
      ...
      <footer className="w-full max-w-md mx-auto text-center text-gray-400 text-sm mt-8 pb-4">
         <div className="w-full sm:w-auto rounded-full p-px bg-animated-border transition-all duration-300 hover:shadow-stone-500/20 mb-4 transform hover:scale-105">
            <a href={developerWhatsAppUrl} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center justify-center gap-3 w-full px-6 py-3 bg-stone-900/80 text-white font-bold rounded-full">
      ...
      <Modal isOpen={activeModal === 'whatsapp'} onClose={() => { setActiveModal(null); resetWhatsappForm(); }}>
        <div className="text-center">
           <div className="bg-gradient-to-r from-stone-400 via-stone-500 to-stone-600 animate-gradient bg-clip-text text-transparent mx-auto"><WhatsAppIcon className="mx-auto h-12 w-12" /></div>
          <h3 className="mt-2 text-2xl font-bold text-white">Fale com a gente!</h3>
      ...
      <Modal isOpen={activeModal === 'rating'} onClose={() => setActiveModal(null)}>
        <div className="text-center">
          ...
          <h3 className="mt-2 text-2xl font-bold text-white">Sua opinião é importante!</h3>
      ...
      <Modal isOpen={activeModal === 'feedback'} onClose={() => setActiveModal(null)}>
        <div className="text-center">
           <h3 className="text-2xl font-bold text-white">{feedbackContent.title}</h3>
        ...
        <form action="https://formsubmit.co/SEU_EMAIL_AQUI" method="POST" className="mt-6 space-y-4">
           ...
           <div className="rounded-lg p-px bg-animated-border focus-within:shadow-lg focus-within:shadow-stone-500/40 transition-shadow duration-300"><textarea name="feedback" placeholder="Sua mensagem..." rows={4} required className="w-full bg-stone-800/50 border-none rounded-[7px] px-4 py-2 text-white placeholder-stone-400 focus:outline-none resize-none"></textarea></div>
           <button type="submit" className="w-full bg-stone-700 text-white font-bold py-3 px-4 rounded-lg hover:bg-stone-600 transition-colors duration-300">Enviar Feedback</button>
        </form>
      </Modal>
      ...
      <Modal isOpen={activeModal === 'location'} onClose={() => setActiveModal(null)}>
        <div className="text-center">
          <LocationIcon className="mx-auto h-10 w-10 bg-gradient-to-r from-stone-400 via-stone-500 to-stone-600 animate-gradient bg-clip-text text-transparent" />
          <h3 className="mt-2 text-2xl font-bold text-white">Nossa Localização</h3>
        ...
        <a
          ...
          className="mt-6 w-full bg-gradient-to-r from-stone-500 via-stone-600 to-stone-700 animate-gradient text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 hover:shadow-lg hover:shadow-stone-500/40 transform hover:scale-105 flex items-center justify-center gap-2"
        >
          Ver no Google Maps
        </a>
      </Modal>
      ...
      <Modal isOpen={activeModal === 'catalogList'} onClose={() => setActiveModal(null)}>
        <h3 className="text-2xl font-bold text-white text-center mb-6">Top 10 da Temporada</h3>
        <div className="grid grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto pr-2">
          {CATALOG_ITEMS.map(item => (
            <div key={item.id} onClick={() => openCatalogItem(item)} className="cursor-pointer group">
              <div className="rounded-lg p-px bg-animated-border"><div className="bg-stone-800/50 rounded-[7px] p-2">
                  ...
                  <p className="text-xs text-stone-300">{item.price}</p>
              </div></div>
            </div>
          ))}
        </div>
      ...
      {selectedItem && (
        <Modal isOpen={activeModal === 'catalogDetail'} onClose={() => setActiveModal('catalogList')}>
          ...
          <h3 className="text-xl font-bold text-white mt-4">{selectedItem.name}</h3>
          <p className="text-lg font-semibold text-stone-300 mt-1">{selectedItem.price}</p>
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {selectedItem.availableSizes.map(size => (
                <span key={size} className="px-3 py-1 bg-stone-700/50 text-xs font-medium rounded-full">{size}</span>
              ))}
            </div>
          </div>
          <div className="flex gap-4 mt-6">
             <button onClick={() => setActiveModal('catalogList')} className="w-1/2 bg-stone-700 text-white font-bold py-3 px-4 rounded-lg hover:bg-stone-600 transition-colors duration-300">Voltar</button>
             <button onClick={() => { setCatalogFormSize(selectedItem.availableSizes[0] || ''); setActiveModal('catalogForm'); }} className="w-1/2 bg-gradient-to-r from-stone-500 via-stone-600 to-stone-700 animate-gradient text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 hover:shadow-lg hover:shadow-stone-500/40 transform hover:scale-105">Tenho Interesse</button>
          </div>
        </Modal>
      )}
      ...
      {selectedItem && (
        <Modal isOpen={activeModal === 'catalogForm'} onClose={() => {setActiveModal(null); resetCatalogForm();}}>
          <h3 className="text-xl font-bold text-white text-center">Interesse em: <br/> <span className="text-stone-300">{selectedItem.name}</span></h3>
          <div className="mt-6 space-y-3 text-left">
            <div className="rounded-lg p-px bg-animated-border"><input type="text" placeholder="Seu nome" value={catalogFormName} onChange={e => setCatalogFormName(e.target.value)} className="w-full bg-stone-800/50 border-none rounded-[7px] px-4 py-2 text-white placeholder-stone-400 focus:outline-none" /></div>
            <div className="rounded-lg p-px bg-animated-border"><input type="text" placeholder="Sua idade" value={catalogFormAge} onChange={e => setCatalogFormAge(e.target.value)} className="w-full bg-stone-800/50 border-none rounded-[7px] px-4 py-2 text-white placeholder-stone-400 focus:outline-none" /></div>
            <div className="rounded-lg p-px bg-animated-border"><select value={catalogFormSize} onChange={e => setCatalogFormSize(e.target.value)} className="w-full bg-stone-800/50 border-none rounded-[7px] px-4 py-2 text-white focus:outline-none appearance-none"><option value="" disabled>Selecione o tamanho</option>{selectedItem.availableSizes.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
            <div>
                <label className="text-sm font-medium text-stone-300">Forma de Pagamento:</label>
                <div className="flex flex-wrap gap-x-4 gap-y-2 mt-2">
                    {['Pix', 'Cartão de Crédito', 'Dinheiro'].map(method => (
                        <label key={method} className="flex items-center space-x-2 text-white text-sm cursor-pointer"><input type="checkbox" checked={catalogFormPayment.includes(method)} onChange={() => handlePaymentChange(method)} className="form-checkbox h-4 w-4 rounded bg-stone-700 border-stone-600 text-stone-500 focus:ring-stone-500" /><span>{method}</span></label>
                    ))}
                </div>
            </div>
            <div className="rounded-lg p-px bg-animated-border"><textarea placeholder="Observações (opcional)" value={catalogFormNotes} onChange={e => setCatalogFormNotes(e.target.value)} rows={2} className="w-full bg-stone-800/50 border-none rounded-[7px] px-4 py-2 text-white placeholder-stone-400 focus:outline-none resize-none"></textarea></div>
          </div>
