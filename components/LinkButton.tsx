...
const LinkButton: React.FC<LinkButtonProps> = ({ href, onClick, text, icon }) => {
  const wrapperClasses = "group rounded-xl p-px bg-animated-border transition-all duration-300 transform hover:scale-105 hover:shadow-gray-400/20";
  const contentClasses = "w-full h-full flex items-center bg-neutral-900/80 backdrop-blur-sm rounded-[11px] p-3 sm:p-4 text-white";

  const content = (
    <div className={contentClasses}>
      <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center text-neutral-300 group-hover:text-white transition-colors duration-300">
        {icon}
      </div>
